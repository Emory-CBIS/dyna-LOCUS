...
#' @param rho A parameter controls the similarity between initial ica results and reconstructed results
#' @param R A numeric vector of pre-specifying ranks of latent sources
...
dlocus_initial = function(Y, q, V, rho = 0.95, R = NULL, maxIter = 100){ 
  ...
  for(i in 1:q){
    ...
    # ?question: why add mean(ICcorr$S[,i]) in the diagonal, not 1's
